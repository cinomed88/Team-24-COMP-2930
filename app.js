...
app.get('/ajax-GET-History', function (req, res) {
    ...
    let qs2 = 'SELECT MATCH.match_id, lat, lng, time, date, sport, score FROM MATCH LEFT JOIN MATCH_PARTICIPANTS ON MATCH_PARTICIPANTS.match_id = MATCH.match_id WHERE MATCH_PARTICIPANTS.user_id =' + '\'' + q.query["name"] + '\'' + 'ORDER BY date, time DESC';
...
WHERE MATCH_PARTICIPANTS.user_id = '${userId}' AND MATCH.date < '${date}'`, { model: matches }).then(function(match){
       ...
       console.log("ERROR WHEN OBTAINING RECENT MATCHES: ", err); 
    });
